...
- State v3 format: `{"version": 3, "monday_items_created": {"monday_item_id": {"board": "SAST", "finding_ids": ["id1", "id2"]}}, "daily": {...}}`. Keyed by monday.com item ID; `finding_ids` lists all Semgrep findings mapped to that item. v1 and v2 are auto-migrated on load. `synced_finding_ids(state)` returns a flat set for O(1) dedup lookups. Filters never modify state — they gate new fetches only.
